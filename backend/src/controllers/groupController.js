...
async function create(req, res) {
  const group = await groupService.createGroup(req.body.name.trim(), req.userId);
...
async function join(req, res) {
  try {
    const group = await groupService.joinGroup(req.body.inviteCode, req.userId);
    res.json(group);
  } catch (err) {
...
async function today(req, res) {
  try {
    const data = await groupService.getTodayStatus(req.params.id, req.userId);
    res.json(data);
  } catch (err) {
    res.status(err.status || 500).json({ error: err.message });
  }
